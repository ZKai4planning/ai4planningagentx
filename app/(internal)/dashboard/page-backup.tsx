...
const PROJECTS: ProjectItem[] = [
  {
    ...
    customer: "Zafer Khan",
    serviceName: "Householder Planning",
    councilName: "Tower Hamlets Council",
    agentXName: "Olivia Carter",
    agentYName: "Daniel Brooks",
    status: "active",
    priority: "high",
    completion: 45,
    waitingOn: "customer",
  ...
  },
  {
    ...
    customer: "Emma Lewis",
    serviceName: "Lawful Development Certificate",
    councilName: "Camden Council",
    agentXName: "Maya Thompson",
    agentYName: "Ethan Reed",
    status: "attention",
    priority: "high",
    completion: 62,
    waitingOn: "agent_x",
  ...
  },
  {
    ...
    agentXName: "Sophia Green",
    agentYName: "Noah Bennett",
    status: "active",
    priority: "medium",
    completion: 58,
    waitingOn: "agent_y",
  ...
  },
  {
    ...
    customer: "Nadia Ali",
    serviceName: "Building Regulations",
    councilName: "Croydon Council",
    agentXName: "Aiden Scott",
    agentYName: "Grace Foster",
    status: "active",
    priority: "low",
    completion: 76,
    waitingOn: "none",
  ...
  },
  {
    ...
    agentXName: "Chloe Morgan",
    agentYName: "Lucas Hayes",
    status: "completed",
    priority: "low",
    completion: 100,
    waitingOn: "none",
  ...
  },
  {
    ...
    customer: "Ava Brown",
    serviceName: "Full Planning Permission",
    councilName: "Lambeth Council",
    agentXName: "Isla Turner",
    agentYName: "Benjamin Clark",
    status: "attention",
    priority: "medium",
    completion: 38,
    waitingOn: "customer",
  ...
  const filteredProjects = useMemo(() => {
    ...
    return PROJECTS.filter((p) => {
      ...
      const searchMatch =
        ...
        p.councilName.toLowerCase().includes(q) ||
        p.agentXName.toLowerCase().includes(q) ||
        p.agentYName.toLowerCase().includes(q)
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center">
            <div className="relative w-full sm:w-auto">
              ...
              <input
                ...
                placeholder="Search by ID, name, customer, council"
                className="w-full rounded-lg border bg-white py-2 pl-8 pr-3 text-sm sm:w-[280px]"
              />
            </div>
            <select
              value={statusFilter}
        ...
        <div className="space-y-4 lg:hidden">
          {filteredProjects.length === 0 ? (
            <div className="rounded-xl border border-dashed bg-slate-50 px-4 py-6 text-center text-sm text-slate-500">
              No projects match your search and status filters.
            </div>
          ) : (
            filteredProjects.map((p) => (
              <div key={p.id} className="rounded-xl border bg-slate-50 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-bold text-slate-900">{p.id}</p>
                    <p className="mt-1 text-sm text-slate-700">{p.name}</p>
                  </div>
                ...
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-800">{p.customer}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Service Name</p>
                    <p className="mt-1 text-sm text-slate-800">{p.serviceName}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Council Name</p>
                    <p className="mt-1 text-sm text-slate-800">{p.councilName}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Agent X Name</p>
                    <p className="mt-1 text-sm text-slate-800">{p.agentXName}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Agent Y Name</p>
                    <p className="mt-1 text-sm text-slate-800">{p.agentYName}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Completion</p>
                    <p className="mt-1 text-sm font-semibold text-slate-800">{p.completion}%</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Waiting On</p>
                    <p className="mt-1 text-sm capitalize text-slate-800">{p.waitingOn.replace("_", " ")}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Last Activity</p>
                    <p className="mt-1 text-sm text-slate-800">{p.lastActivityHoursAgo}h ago</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Unread Messages</p>
                    <p className="mt-1 text-sm text-slate-800">{p.unreadMessages}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link
                    href={`/projects/${p.id}/workspace/project`}
                    className="rounded-md border bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100"
                  >
                    Project
                  </Link>
                  <Link
          ...
          <table className="w-full min-w-[1480px] text-sm">
            <thead>
              <tr className="border-b text-left text-xs uppercase tracking-wider text-slate-500">
                ...
                <th className="pb-2">Council Name</th>
                <th className="pb-2">Agent X Name</th>
                <th className="pb-2">Agent Y Name</th>
              ...
              {filteredProjects.length === 0 ? (
                ...
                filteredProjects.map((p) => (
                  <tr key={p.id} className="border-b last:border-0">
                    ...
                    <td className="py-3 text-slate-700">{p.customer}</td>
                    <td className="py-3 text-slate-700">{p.serviceName}</td>
                    <td className="py-3 text-slate-700">{p.councilName}</td>
                    <td className="py-3 text-slate-700">{p.agentXName}</td>
                    <td className="py-3 text-slate-700">{p.agentYName}</td>
                    <td className="py-3">
                      <StatusBadge value={p.status} />
                    </td>
                    <td className="py-3 font-semibold text-slate-700">{p.completion}%</td>
                    <td className="py-3 capitalize text-slate-600">{p.waitingOn.replace("_", " ")}</td>
